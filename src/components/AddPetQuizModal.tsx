...
const PET_TYPE_OPTIONS = [
  { id: "dog", label: "🐶 Dog", labelRu: "🐶 Собака", emoji: "🐶", species: "Dog", speciesRu: "Собака", fallbackImage: phoneSnapDogImg },
  { id: "cat", label: "🐱 Cat", labelRu: "🐱 Кошка", emoji: "🐱", species: "Cat", speciesRu: "Кошка", fallbackImage: phoneSnapCatImg },
  { id: "rabbit", label: "🐰 Rabbit", labelRu: "🐰 Кролик", emoji: "🐰", species: "Rabbit", speciesRu: "Кролик", fallbackImage: phoneSnapRabbitImg },
  { id: "rodent", label: "🐹 Rodent", labelRu: "🐹 Грызун", emoji: "🐹", species: "Rodent", speciesRu: "Грызун", fallbackImage: phoneSnapRodentImg },
  { id: "bird", label: "🐦 Bird", labelRu: "🐦 Птица", emoji: "🐦", species: "Bird", speciesRu: "Птица", fallbackImage: phoneSnapBirdImg },
  { id: "reptile", label: "🐢 Reptile", labelRu: "🐢 Рептилия", emoji: "🐢", species: "Reptile", speciesRu: "Рептилия", fallbackImage: phoneSnapReptileImg },
  { id: "another", label: "🦄 Another", labelRu: "🦄 Другой", emoji: "🦄", species: "Another", speciesRu: "Другой", fallbackImage: phoneSnapUnicornImg },
];

export default function AddPetQuizModal({ isOpen, onClose, onAddPet, currentLanguage = "ru" }: AddPetQuizModalProps) {
  ...
  const handleNextStep = () => {
    ...
    if (step < 4) {
      ...
      const finalName = petNameInput.trim() || (isRu ? matched.speciesRu : matched.species);

      const newPet: Pet = {
        id: `pet_${Date.now()}_${Math.random().toString(36).substring(2, 7)}`,
        name: finalName,
        type: (selectedPetType || "dog") as PetType,
        species: isRu ? matched.speciesRu : matched.species,
        emoji: matched.emoji,
        image: matched.fallbackImage,
        breed: isRu ? matched.speciesRu : matched.species,
        age: selectedPetAge || "1-3",
        sex: selectedPetSex || "Male",
        weight: selectedPetType === "dog" ? "6.8 кг" : selectedPetType === "cat" ? "4.2 кг" : "1.2 кг",
        isCustom: true,
      };
  ...
  const getAgeOptions = () => {
    if (isRu) {
      if (selectedPetType === "rodent") {
        return ["Младше 1 года", "1-3 года", "4+ года", "Точно не знаю"];
      } else if (selectedPetType === "rabbit") {
        return ["Младше 1 года", "1-3 года", "4-7 лет", "8-12+ лет", "Точно не знаю"];
      } else if (selectedPetType === "reptile") {
        return ["Младше 1 года", "1-3 года", "4-7 лет", "8-12 лет", "13-100 лет", "Точно не знаю"];
      }
      return ["Младше 1 года", "1-3 года", "4-7 лет", "8-12 лет", "13+ лет", "Точно не знаю"];
    }
    if (selectedPetType === "rodent") {
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed inset-0 z-[100] w-full h-[100dvh] bg-[#f5f4fa] dark:bg-[#000000] flex flex-col justify-between pt-5 pb-3 font-sf select-none overflow-hidden shadow-2xl"
        >
        ...
        <AnimatePresence mode="wait" custom={quizDirection}>
            ...
            >
              <div className="mt-3 mb-5 overflow-hidden">
                <h1 className="text-[27px] sm:text-[29px] font-bold tracking-tight leading-tight text-transparent bg-clip-text bg-gradient-to-t from-[#424242] to-[#000000] dark:from-[#D1D1D6] dark:to-[#FFFFFF] text-left w-full select-none">
                  {isRu ? "Кто ваш питомец?" : "Who is your pet?"}
                </h1>
              </div>
          ...
          {step === 2 && (
            ...
            >
              <div className="mt-3 mb-5 overflow-hidden">
                <h1 className="text-[27px] sm:text-[29px] font-bold tracking-tight leading-tight text-transparent bg-clip-text bg-gradient-to-t from-[#424242] to-[#000000] dark:from-[#D1D1D6] dark:to-[#FFFFFF] text-left w-full select-none">
                  {isRu ? "Как зовут вашего питомца?" : "What is your pet's name?"}
                </h1>
              </div>
              ...
              <div className="flex-1 pb-2 flex flex-col justify-center">
                <input
                  ...
                  placeholder={isRu ? "Кличка питомца" : "Pet’s name"}
          ...
          {step === 3 && (
            ...
            >
              <div className="mt-3 mb-4 overflow-hidden">
                <h1 className="text-[27px] sm:text-[29px] font-bold tracking-tight leading-tight text-transparent bg-clip-text bg-gradient-to-t from-[#424242] to-[#000000] dark:from-[#D1D1D6] dark:to-[#FFFFFF] text-left w-full select-none">
                  {isRu ? `Сколько лет вашему питомцу?` : `What is your ${currentSpeciesTitle} age?`}
                </h1>
              </div>
          ...
          {step === 4 && (
            ...
            >
              <div className="mt-3 mb-4 overflow-hidden">
                <h1 className="text-[27px] sm:text-[29px] font-bold tracking-tight leading-tight text-transparent bg-clip-text bg-gradient-to-t from-[#424242] to-[#000000] dark:from-[#D1D1D6] dark:to-[#FFFFFF] text-left w-full select-none">
                  {isRu ? "Пол питомца" : `What is your ${currentSpeciesTitle} sex?`}
                </h1>
              </div>
              ...
              <div className="flex-1 pb-2 flex flex-col">
                ...
                >
                  {(isRu
                    ? [
                        { id: "Male", label: "Самец (Мальчик)" },
                        { id: "Female", label: "Самка (Девочка)" },
                        { id: "Prefer not to say", label: "Не указывать" },
                      ]
                    : [
                        { id: "Male", label: "Male" },
                    ...
                    const isSelected = selectedPetSex === opt.id;
                    return (
                      <motion.button
                        key={opt.id}
                        variants={cardItemVariants}
                        onClick={() => handleSelectPetSex(opt.id)}
                        whileTap={{ scale: 0.985 }}
      ...
      <div className="w-full max-w-md mx-auto px-6 pb-6 pt-2 flex-none">
        ...
        >
          {step === 4 ? (isRu ? "Добавить питомца" : "Add Pet") : (isRu ? "Продолжить" : "Continue")}
        </motion.button>
      </div>
    </motion.div>
      )}
