...
const METRICS_TRANSLATIONS: Record<string, {
  ...
  ru: {
    ...
    Water: "Вода",
    Play: "Игры",
    DrinkingMode: "Питьевой режим",
    DrunkToday: "Выпито сегодня",
    TargetGoal: "Рекомендуемая норма",
    Done: "Готово",
    ReduceWater: "Убавить 50 мл",
    Medication: "Прием лекарств",
    NewMedicine: "Новое лекарство",
    MedicineName: "Название",
    AddToSchedule: "Добавить в график",
    TodaysSchedule: "График на сегодня",
    NoMeds: "Лекарств на сегодня не назначено",
  ...
  return (
    <div id="metrics-dashboard-grid" className="w-full max-w-md px-2.5 space-y-4 pb-8 select-none">
                  ...
                  <div className="flex-1 flex items-center justify-between pr-1">
                    <div className="flex flex-col">
                      <span className="text-[16px] font-bold text-[#1c1c1e] tracking-tight leading-snug">
                        {report.statusLabel || (scanScore >= 80 ? (lang === "ru" ? "Отличное" : "Healthy") : scanScore >= 60 ? (lang === "ru" ? "В норме" : "Moderate") : (lang === "ru" ? "Внимание" : "Attention"))}
                      </span>
                      <span className="text-[13px] font-medium text-zinc-400 mt-1">
                        {lang === "ru" ? "Общее здоровье" : "Health Score"}
                      </span>
                    </div>
      ...
      <AnimatePresence>
        {showMedsSheet && (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-md z-[100] flex items-end justify-center">
            <div className="absolute inset-0" onClick={() => setShowMedsSheet(false)}></div>
            <motion.div
              drag="y"
              ...
              className="w-full max-w-md bg-white rounded-t-[2.5rem] p-6 pt-5 shadow-2xl relative z-10 flex flex-col max-h-[85vh]"
              id="medications-bottom-sheet"
            >
              <div className="flex justify-between items-center mb-5 cursor-grab active:cursor-grabbing touch-none select-none">
        ...
        {showRemindersSheet && (
          <div className="fixed inset-0 z-[100] flex flex-col justify-end overflow-hidden">
            ...
            <motion.div
              ...
              className="relative w-full max-w-md mx-auto bg-white dark:bg-gradient-to-b dark:from-[#202022] dark:to-[#1C1C1E] text-black dark:text-white rounded-t-[40px] px-6 pt-5 pb-8 shadow-[0_-16px_48px_rgba(0,0,0,0.15)] flex flex-col max-h-[85vh] z-10"
              id="reminders-popup"
            >
              {/* Header */}
              <div className="flex justify-between items-center mb-6 cursor-grab active:cursor-grabbing touch-none select-none">
                        ...
                        <div className="flex items-center">
                          {editingReminderId === rem.id ? (
                            ...
                            <span
                              ...
                              title={lang === "ru" ? "Нажмите, чтобы изменить время" : "Click to edit time"}
                            >
                              {rem.time}
                            </span>
                          )}
        ...
        {showNewReminderModal && (
          <div className="fixed inset-0 z-[100] flex flex-col justify-end overflow-hidden">
            ...
            <motion.div
              ...
              exit={{ y: "100%" }}
              transition={{ type: "spring", stiffness: 420, damping: 36 }}
              className="relative w-full max-w-md mx-auto bg-white dark:bg-gradient-to-b dark:from-[#202022] dark:to-[#1C1C1E] text-black dark:text-white rounded-t-[40px] px-6 pt-5 pb-8 shadow-[0_-16px_48px_rgba(0,0,0,0.15)] flex flex-col z-10"
      ...
      <AnimatePresence>
        {showWaterSheet && (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-md z-[100] flex items-end justify-center">
            <div className="absolute inset-0" onClick={() => setShowWaterSheet(false)}></div>
            <motion.div
              drag="y"
              dragConstraints={{ top: 0 }}
              dragSnapToOrigin={true}
              ...
              exit={{ y: "100%" }}
              transition={{ type: "spring", damping: 25, stiffness: 220 }}
              className="w-full max-w-md bg-white dark:bg-gradient-to-b dark:from-[#202022] dark:to-[#1C1C1E] text-black dark:text-white rounded-t-[2.5rem] p-6 pt-5 shadow-2xl relative z-10 flex flex-col"
      ...
      <AnimatePresence>
        {showStreakPage && (
          <motion.div
            ...
            exit={{ y: "100%" }}
            transition={{ type: "spring", stiffness: 420, damping: 36 }}
            className="fixed inset-0 bg-[#F8F9FA] dark:bg-black z-[100] flex flex-col justify-between p-6 overflow-y-auto select-none text-[#1c1c1e] dark:text-white"
            style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "SF Pro Display", "SF Pro Text", "SF Pro", "San Francisco", "Helvetica Neue", sans-serif' }}
          >
            {/* Header */}
            ...
            <div className="w-full space-y-4 max-w-sm mx-auto">
              ...
              <button
                onClick={() => {
                  const shareText = lang === "ru" 
                    ? `Я поддерживаю серию заботы о питомце уже ${streak} дней подряд в PetAI! 🔥` 
                    : lang === "fr"
                  ...
                  if (navigator.clipboard) {
                    ...
                    alert(lang === "ru" ? "Серия скопирована в буфер обмена!" : "Streak copied to clipboard!");
                  } else {
                    alert(shareText);
                  }
                }}
              ...
              >
                ...
                {lang === "ru" ? "Поделиться серией" : lang === "fr" ? "Partager la série" : lang === "de" ? "Serie teilen" : lang === "es" ? "Compartir racha" : lang === "it" ? "Condividi serie" : lang === "ja" ? "記録をシェアする" : lang === "ko" ? "기록 공유하기" : lang === "zh" ? "分享连续天数" : lang === "pt-BR" ? "Compartilhar Sequência" : "Share Streak"}
              </button>
            </div>
          </motion.div>
        )}
          ...
          >
              ...
              <button
                onClick={() => {
                  const alertMsg = lang === "ru"
                    ? `Этот уровень отражает общее состояние здоровья вашего питомца.\n\nТекущая оценка: ${hasScans ? `${score}%` : "Нет данных"}`
              ...
              <div className="mt-7 text-center max-w-sm px-4">
                <h2 className="text-[28px] font-bold text-[#1c1c1e] dark:text-white tracking-tight leading-tight">
                  {(() => {
                    ...
                    if (score >= 75) return lang === "ru" ? "В норме" : "Healthy";
                    if (score >= 60) return lang === "ru" ? "Удовлетворительное" : "Moderate";
                ...
                <p className="text-[14px] font-medium text-zinc-500 dark:text-[#98989D] mt-2 leading-relaxed">
                  {(() => {
                    if (!hasScans) {
                      return lang === "ru" 
                        ? "Сделайте первый анализ здоровья вашего питомца по фотографии, чтобы увидеть подробный статус."
                        : "Scan your pet's health with a photo to see a detailed status report.";
                    }
                    if (latestReport?.summary) return latestReport.summary;
                    if (score >= 90) {
                      return lang === "ru"
                        ? "Все показатели здоровья вашего питомца находятся в отличном состоянии. Регулярные проверки помогают вовремя обнаружить малейшие изменения."
                        : "All health indicators of your pet are in excellent condition. Regular checks help identify changes early.";
                    }
                    if (score >= 75) {
                      return lang === "ru"
                        ? "Показатели здоровья вашего питомца в пределах нормы. Продолжайте регулярный уход и полноценное питание."
                        : "Your pet's health indicators are within the normal range. Continue regular care and balanced nutrition.";
                    }
                    if (score >= 60) {
                      return lang === "ru"
                        ? "Общее состояние здоровья удовлетворительное. Есть незначительные замечания к питьевому режиму или уровню активности."
                        : "Overall health is satisfactory. There are minor remarks regarding hydration or activity levels.";
                    }
                    return lang === "ru"
                      ? "Рекомендуется уделить повышенное внимание здоровью питомца и ознакомиться со списком рекомендаций."
                      : "It is recommended to pay extra attention to your pet's health and review our recommendations.";
                  })()}
                </p>
              </div>
            </div>
            ...
            <div className="w-full space-y-4 max-w-sm mx-auto">
              ...
              <button
                onClick={() => {
                  const shareText = lang === "ru" 
                    ? `Результат здоровья моего питомца: ${hasScans ? `${score}%` : "Ожидание проверки"}! Серия заботы: ${streak} дней подряд. Проверьте своего питомца в Pet AI! 🐾`
                    : `My pet health score is ${hasScans ? `${score}%` : "Pending"}! Streak: ${streak} days. Scan your pet too with Pet AI! 🐾`;
                  if (navigator.clipboard) {
                    navigator.clipboard.writeText(shareText);
                    alert(lang === "ru" ? "Статус здоровья скопирован в буфер обмена!" : "Health status copied to clipboard!");
                  } else {
                    alert(shareText);
                  }
                }}
