...
export default function App() {
  const [currentLanguage, setCurrentLanguage] = useState<string>(() => {
    const saved = localStorage.getItem("pethealth_language") || "en";
    return saved === "ru" ? "en" : saved;
  });
  const t = TRANSLATIONS[currentLanguage] || TRANSLATIONS.en;
  const [showLanguagePopup, setShowLanguagePopup] = useState<boolean>(false);
  const [showProfileModal, setShowProfileModal] = useState<boolean>(false);
  const [isOnboarded, setIsOnboarded] = useState<boolean>(() => {
    return sessionStorage.getItem("pethealth_session_onboarded") === "true";
  });
  ...
  // Splash Screen State (1.5 seconds white screen with centered Petkit name fade-in)
  ...
  const handleAddPet = (newPet: Pet) => {
    const updated = [...pets.filter((p) => p.id !== newPet.id), newPet];
    setPets(updated);
    setActivePetId(newPet.id);
    try {
  ...
  if (!isOnboarded) {
          ...
          <AnimatePresence>
            {showLanguagePopup && (
              <div className="fixed inset-0 z-[100] flex flex-col justify-end overflow-hidden">
                ...
                {/* Highly tactile, fully responsive, spring-loaded drag-to-dismiss sheet matching camera preview */}
                <motion.div
                  drag="y"
                  ...
                  className="relative w-full max-w-md mx-auto bg-white rounded-t-[64px] px-6 pt-6 pb-8 shadow-[0_-16px_48px_rgba(0,0,0,0.22)] flex flex-col z-10 select-none"
                  onClick={(e) => e.stopPropagation()}
                >
                  {/* Header */}
                  <div className="flex justify-between items-center mb-4 w-full cursor-grab active:cursor-grabbing touch-none">
    ...
    const canContinue = 
      ...
      quizStep === 10 ? true :
      quizStep === 11 ? selectedPhotoDays.length > 0 :
          ...
          <AnimatePresence mode="wait" custom={quizDirection}>
            ...
            {quizStep === 10 && (
              <motion.div
                ...
                exit={{ opacity: 0, x: quizDirection === "forward" ? -35 : 35 }}
                transition={{ duration: 0.22, ease: [0.25, 1, 0.5, 1] }}
                className="w-full flex-1 flex flex-col justify-between items-center py-2 select-none"
              >
                {/* Upper / Middle Scanner Container with Black Corner Brackets identical to main page PetScanner */}
                <div className="flex-1 flex flex-col items-center justify-center my-auto w-full">
                  <motion.div 
                    animate={{ scale: [0.94, 1.06, 0.94] }}
                    transition={{ repeat: Infinity, duration: 2.8, ease: "easeInOut" }}
                    className="relative w-[190px] h-[190px] sm:w-[210px] sm:h-[210px] flex items-center justify-center select-none"
                  >
                    {/* Ultra-smooth rounded SVG brackets matching main page PetScanner */}
                    <svg 
                      viewBox="0 0 100 100" 
                      className="absolute inset-0 w-full h-full text-black pointer-events-none select-none z-10" 
                      stroke="currentColor" 
                      strokeWidth="7" 
                      strokeLinecap="round" 
                      strokeLinejoin="round" 
                    ...
                    {/* Content inside Brackets (only if photo captured) */}
                    {capturedFirstPhoto && (
                      <div className="w-[144px] h-[144px] sm:w-[158px] sm:h-[158px] rounded-[24px] overflow-hidden">
                        <img
                          src={capturedFirstPhoto}
                          alt="Pet scan preview"
                          className="w-full h-full object-cover"
                        />
                      </div>
                    )}
                  ...
                  {/* Title Text */}
                  <h1 className="text-[26px] sm:text-[28px] font-bold tracking-tight leading-tight text-transparent bg-clip-text bg-gradient-to-t from-[#424242] to-[#000000] text-center max-w-[280px] mt-6 select-none">
                    Take your pet’s first photo-check
                  </h1>
                </div>
              </motion.div>
            )}
            ...
            {quizStep === 11 && (
              ...
              >
                <div className="mt-3 mb-4 overflow-hidden">
                  <h1 className="text-[27px] sm:text-[29px] font-bold tracking-tight leading-tight text-transparent bg-clip-text bg-gradient-to-t from-[#424242] to-[#000000] text-left w-full select-none">
                    Which days should photo checks be on?
                  </h1>
                </div>
                ...
                <div className="flex-1 pb-2 flex flex-col">
                  ...
                  >
                    {[
                      ...
                      const isSelected = selectedPhotoDays.includes(day);
                      return (
                        <motion.button
                          key={day}
                          variants={cardItemVariants}
                          onClick={() => {
                            if (isSelected) {
                              setSelectedPhotoDays(selectedPhotoDays.filter((d) => d !== day));
                            } else {
                              setSelectedPhotoDays([...selectedPhotoDays, day]);
                            }
                            if (navigator.vibrate) {
                              navigator.vibrate(15);
                            }
          ...
          {quizStep === 9 ? (
            ...
            <div className="flex flex-col items-center gap-2 w-full pb-1">
              <motion.button
                ...
                className="w-full py-[18px] text-[19px] tracking-tight font-bold rounded-[50px] bg-black hover:bg-zinc-900 text-white active:scale-[0.98] transition-colors duration-300 flex items-center justify-center cursor-pointer shadow-lg"
              >
                {capturedFirstPhoto ? t.Continue : "Scan Pet"}
              </motion.button>
  ...
  return (
    <div className="w-full min-h-screen bg-[#f5f4fa] dark:bg-[#000000] text-[#1c1c1e] dark:text-white flex flex-col items-center pt-3.5 pb-28 px-3 select-none overflow-x-hidden relative transition-colors duration-250">
      ...
      <div className="w-full max-w-md flex flex-col items-center space-y-4">
        ...
        {/* Top Header: 'Petkit' brand only on home screen, static and not moving */}
        {activeNavTab === "home" && (
      ...
      {/* Floating iOS-style Bottom Navigation Bar with swipe & drag support, smooth blur & plus-to-cross interaction */}
      <FloatingBottomNav
        ...
        {showProfileModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/40 backdrop-blur-xs">
            <motion.div
              ...
              className="bg-white w-full max-w-sm rounded-3xl p-6 shadow-2xl border border-zinc-100 flex flex-col"
            >
              <div className="flex items-center justify-between mb-5">
                ...
                <button
                  ...
                  className="w-8 h-8 rounded-full bg-zinc-100 hover:bg-zinc-200 flex items-center justify-center text-zinc-500 hover:text-black transition-colors cursor-pointer"
                >
